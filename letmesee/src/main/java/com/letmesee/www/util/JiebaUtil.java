    ...
    public static Map<String,WordPacking> jiebaWebText(String title, String content){
        ...
        for(int i=0;i<titleX.size();i++){
            ...
            if(map.containsKey(keyword)){
                ...
                //计算tf
                int tt = w.getTt();
                double tf = (double) tt/sum;
                w.setTf(tf);
            }else{
                List<Integer> p = new ArrayList<>();
                p.add(pos);
                WordPacking w = new WordPacking(keyword,1,0,(double) 1/sum,p);
                map.put(keyword,w);
            }
        }
        ...
        for(int i=0;i<contentX.size();i++){
            ...
            if(map.containsKey(keyword)){
                ...
                int tt = w.getTt();
                int ct = w.getCt();
                double tf = (double) (tt+ct*10)/sum;
                w.setTf(tf);
            }else{
                List<Integer> p = new ArrayList<>();
                p.add(pos);
                WordPacking w = new WordPacking(keyword,0,1,(double) 10/sum,p);
                map.put(keyword,w);
            }
        }
